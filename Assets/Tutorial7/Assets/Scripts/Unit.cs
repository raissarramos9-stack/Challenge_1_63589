...
public abstract class Unit : MonoBehaviour, UIMainScene.IUIInfoContent
{
    ...
    private void Start()
    {
        if (MainManager07.Instance != null)
        {
            SetColor(MainManager07.Instance.TeamColor);
        }
    }
    ...
    // Implementing the IUIInfoContent interface so the UI knows it should display the UI when this is clicked on.
